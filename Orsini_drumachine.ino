...
void control(){
    ...
    if (divup.update()) { 
        if (divup.read() == HIGH){
         if(divTemp==1){ divTemp++;
        }else{
            divTemp=(divTemp+2)%16;
         }
...
void Play() {
   ...
   if(Position==0) division=divTemp;
   if(Position==0) global++;
   randFill = map(analogRead(5), 0, 1023, 1, 16);                // remap it to values similar to those in cluster configurations
   ...
   Ymap = map(Ymap, 0, 1023, -100, 100);   
   if(global%randFill==0){
    if(randFill<16){
    // BDfreq+=random(-128, 128);
    // SDfreq+=random(-128, 128);
    // HHfreq+=random(-128, 128);
    // Xmap+=random(-50, 50);
    // Ymap+=random(-50, 50);
    }
   }              // remap around zero
  ...
  ///////////////////////////////// above are the equations for interpolating between values. Nothing fancy.
   ...
   if (BDrum > BDfreq) playNote(36+note, 127, 1000, 10); 
   if (SDrum > SDfreq) playNote(37+note, 127, 1000, 10); 
   if (HHats > HHfreq) playNote(38+note, 127, 1000, 10);         // sending midi notes
